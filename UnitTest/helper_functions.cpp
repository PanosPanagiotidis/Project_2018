...
Table_Info* init_table_info2(uint64_t** a, uint64_t* b,int relNum, int relID, int size,threadpool* THREAD_POOL)		// Initializes the variables and structs of table info, a=keys, b=payloads
{
	...
	for(int i = 0 ;i < jobs ; i++){
		...
		arg_table[i]->rowId = a[relID];

		arg_table[i]->rids = NULL;
		arg_table[i]->relNum = relNum;
		if( relNum != 0)
			arg_table[i]->rids = a;
	...
	if( relNum != 0 )
	{
		ti->rids = new uint64_t *[relNum];
		for( int k = 0; k < relNum; k++)
			ti->rids[k] = new uint64_t[size];
	}
	...
	for(int i = 0 ;i < jobs ; i++){
		...
		harg_table[i]->relNum = relNum;
		harg_table[i]->rids = NULL;
		harg_table[i]->stored_rids = NULL;
		if( relNum != 0)
		{
			harg_table[i]->stored_rids = ti->rids;
			harg_table[i]->rids = a;
		}
		...
		if(ti->histogram[i] != 0){ // Bucket is not existant
			...
			for(int j = 0 ; j < ti->histogram[i] ; j++){
				...
				if( relNum != 0 )
				{
					...
					{
						ti->bck_array->bck[i]->tuplesArray[j]->rids[l] = (ti->rids[l][prg]);
					}
				}
